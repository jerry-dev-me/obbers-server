...
const queryFunc = {
  unread: async (writerId, reportId) => {
    return await q.features.report.update.status.unread(writerId, reportId);
  },
  inProcess: async (writerId, reportId) => {
    return await q.features.report.update.status.inProcess(writerId, reportId);
  },
  closed: async (writerId, reportId) => {
    return await q.features.report.update.status.closed(writerId, reportId);
  },
};

// const queryFunc = async (writerId, reportId) =>
//   await q.features.report.update.status(writerId, reportId);

describe("\n query function: report.update.status()", async function() {
  this.timeout(0);
