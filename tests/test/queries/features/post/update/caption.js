...
const queryFunc = async (writerId, postId, updatedCaption) =>
  await q.features.post.update.caption(writerId, postId, updatedCaption);

describe("\n query function: post.update.caption()", async function() {
  this.timeout(0);
  ...
  describe("\n ADMIN only", async () => {
    it("account permissions ADMIN", async () => {
      ...
      const funcResults = await queryFunc(testUserId, postId, "new_caption");
  ...
  describe("\n self only", async () => {
    it.only("document from self", async () => {
