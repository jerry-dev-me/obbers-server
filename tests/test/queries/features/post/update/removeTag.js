...
const queryFunc = async (writerId, postId, tagId, userId) =>
  await q.features.post.update.removeTag(writerId, postId, tagId, userId);

describe("\n query function: post.update.removeTag()", async function() {
  this.timeout(0);
  ...
  describe("\n ADMIN only", async () => {
    it("account permissions ADMIN", async () => {
      ...
      const funcResults = await queryFunc(testUserId, postId, tagId, userId);
