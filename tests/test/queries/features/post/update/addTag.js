...
const queryFunc = async (writerId, postId, tagFields) =>
  await q.features.post.update.addTag(writerId, postId, tagFields);

describe("\n query function: post.update.addTag()", async function() {
  this.timeout(0);
  ...
  describe("\n document model fields validation", async () => {
    it("valid fields", async () => {
      const postId = data;
      const fields = data;
      const funcResults = await queryFunc(testUserId, postId, fields);
