...
const queryFunc = async (writerId, collectionId, newName) =>
  await q.features.collection.update.name(writerId, collectionId, newName);

describe("\n query function: collection.update.name()", async function() {
  this.timeout(0);
  ...
  describe("\n self only", async () => {
    it("document from self", async () => {
      ...
      const funcResults = await queryFunc(testUserId, collectionId, "nameX");
