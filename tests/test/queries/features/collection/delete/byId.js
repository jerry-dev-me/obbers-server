...
const queryFunc = async (writerId, collectionId) =>
  await q.features.collection.delete.byId(writerId, collectionId);

describe("\n query function: collection.delete.byId()", async function() {
  this.timeout(0);
  ...
  describe("\n ADMIN only", async () => {
    ...
    it("account permissions not ADMIN", async () => {
      ...
      const collectionId = data.adminOnly.user1.collection2.id;
      const funcResults = await queryFunc(testUserId, collectionId);
      logger.log(lG, lS, null, { funcResults });
      assert(funcResults instanceof Error);
