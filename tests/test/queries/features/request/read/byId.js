...
const queryFunc = async (readerId, requestId) =>
  await q.features.request.read.byId(readerId, requestId);

describe("\n query function: request.read.byId()", async function() {
  this.timeout(0);
  ...
  describe("\n ADMIN only", async () => {
    it("account permissions ADMIN", async () => {
      ...
      const requestId = data.adminOnly.user1.request1.id;
