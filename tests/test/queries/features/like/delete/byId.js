...
const queryFunc = async (writerId, likeId) =>
  await q.features.like.delete.byId(writerId, likeId);

describe("\n query function: like.delete.byId()", async function() {
  this.timeout(0);
  ...
  describe("\n ADMIN only", async () => {
    it("account permissions ADMIN", async () => {
      ...
      const funcResults = await queryFunc(testUserId, likeId);
  ...
  describe("\n self only", async () => {
    it("document from self", async () => {
      const likeId = data.selfOnly.testUser.postLike1.id;
      const funcResults = await queryFunc(testUserId, likeId);
      logger.log(lG, lS, null, { funcResults });
      assert(!(funcResults instanceof Error));
    });
    it("document from self", async () => {
      const likeId = data.selfOnly.testUser.commentLike1.id;
      const funcResults = await queryFunc(testUserId, likeId);
      logger.log(lG, lS, null, { funcResults });
      assert(!(funcResults instanceof Error));
    });
    it("document from self", async () => {
      const likeId = data.selfOnly.testUser.responseLike1.id;
      const funcResults = await queryFunc(testUserId, likeId);
      logger.log(lG, lS, null, { funcResults });
      assert(!(funcResults instanceof Error));
    });
    it("document from self", async () => {
      const likeId = data.selfOnly.testUser.post1.postLike1.id;
      const funcResults = await queryFunc(testUserId, likeId);
      logger.log(lG, lS, null, { funcResults });
      assert(!(funcResults instanceof Error));
    });
    it("document from self", async () => {
      const likeId = data.selfOnly.testUser.comment1.commentLike1.id;
      const funcResults = await queryFunc(testUserId, likeId);
      logger.log(lG, lS, null, { funcResults });
      assert(!(funcResults instanceof Error));
    });
    it("document from self", async () => {
      const likeId = data.selfOnly.testUser.response1.responseLike1.id;
      const funcResults = await queryFunc(testUserId, likeId);
      logger.log(lG, lS, null, { funcResults });
      assert(!(funcResults instanceof Error));
    });
    it("document from other user", async () => {
      const likeId = data.selfOnly.user1.postLike1.id;
      const funcResults = await queryFunc(testUserId, likeId);
      logger.log(lG, lS, null, { funcResults });
      assert(funcResults instanceof Error);
      assert(funcResults.customErr);
    ...
    it("document from other user", async () => {
      const likeId = data.selfOnly.user1.commentLike1.id;
      const funcResults = await queryFunc(testUserId, likeId);
      logger.log(lG, lS, null, { funcResults });
      assert(funcResults instanceof Error);
      assert(funcResults.customErr);
    ...
    it("document from other user", async () => {
      const likeId = data.selfOnly.user1.responseLike1.id;
      const funcResults = await queryFunc(testUserId, likeId);
      logger.log(lG, lS, null, { funcResults });
      assert(funcResults instanceof Error);
      assert(funcResults.customErr);
