...
const queryFunc = async (readerId, likeId) =>
  await q.features.like.read.byId(readerId, likeId);

describe("\n query function: like.read.byId()", async function() {
  this.timeout(0);
  ...
  describe("\n account status", async () => {
    describe("\n self", async () => {
      it("account status is ACTIVE", async () => {
        ...
        const likeId = data.accountStatus.user1.postLike1.id;
        const funcResults = await queryFunc(testUserId, likeId);
        logger.log(lG, lS, null, { funcResults });
        assert(!(funcResults instanceof Error));
      });
