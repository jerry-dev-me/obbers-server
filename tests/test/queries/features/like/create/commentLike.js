...
const queryFunc = async (writerId, commentId) =>
  await q.features.like.create.commentLike(writerId, commentId);

describe("\n query function: like.create.commentLike()", async function() {
  this.timeout(0);
  ...
  // commentLike on commentId that belongs to one of the user refereneces..
  ...
  describe("\n nonexistent documents", async () => {
    it("nonexistent document", async () => {
      const commentId = data.nonexistent.comment1.id;
      const funcResults = await queryFunc(testUserId, commentId);
      logger.log(lG, lS, null, { funcResults });
      assert(funcResults instanceof Error);
      assert(funcResults.customErr);
    ...
    it("nonexistent document", async () => {
      const commentId = data.nonexistent.comment2.id;
      const funcResults = await queryFunc(testUserId, commentId);
      logger.log(lG, lS, null, { funcResults });
      assert(funcResults instanceof Error);
      assert(funcResults.customErr);
